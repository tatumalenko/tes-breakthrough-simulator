...
%D1  = two_point_upwind_uni_D1(0,H,nz,vg);  %
D1  = five_point_biased_upwind_D1(z,vg);
%D2  = three_point_centered_uni_D2(0,H,nz);  %
...
[t,y] = ode15s(@model,tout,y0,options) ;
...
subplot(1,2,2); plot(e.Temp.t,e.Temp.Tgend,'o',ysol.t,ysol.Tgend); ylim([0 80]);
    ...
    function yt = model(t,y)
        ...
        cgt = -vi.*cgz + Dz.*cgzz - kads.*(cg-cge);
        Tgt = -vg.*rhog.*Cpg./eta.*Tgz + k./eta.*Tgzz - 2./Rint.*hw./eta.*(Tg-Tw) - dHads.*kads./eta.*(cg-cge);
    ...
    function M = mass_react % set global variables
